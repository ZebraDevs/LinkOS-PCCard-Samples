...
#Region "Main Form Methods & Events"
    ...
    Private Sub cboPrn_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboPrn.SelectedIndexChanged
        Try
            If (Not _startup) Then
                GetSelectedPrinterConfiguration()
            End If
    ...
    Private Sub GetSelectedPrinterConfiguration()
        Try
            Dim isContact, isMag As Boolean
            Dim errMsg As String = String.Empty

            If (GetPrinterConfiguration(isContact, isMag)) Then
                If (isContact AndAlso rbUSB.Checked) Then
        ...
        Try
            ...
            If (Not (objList Is Nothing) AndAlso (Not String.IsNullOrEmpty(Convert.ToString(objList)))) Then
                ...
                If (rbUSB.Checked) Then
                    ...
                    Next
                    cboPrn.Text = Convert.ToString(cboPrn.Items(0))
                End If
            End If
    ...
    Private Function GetPrinterConfiguration(ByRef isContact As Boolean, ByRef isMag As Boolean) As Boolean
        Try
            Dim errMsg As String = String.Empty

            Dim deviceName As String = ConnectionType

            GetPrinterConfiguration(deviceName, isContact, isMag, errMsg)
    ...
    Private Sub tvTestsInit(ByVal contactlessReader As String, ByVal contactReader As String, _
        ...
        If (rbUSB.Checked) Then
            If (Not String.IsNullOrEmpty(contactReader)) Then
                If (contactReader.ToLower().Contains("acr")) Then
                    tvTests.Nodes.Add("Contact Station")
                    tvTests.Nodes(iNode).Nodes.Add("Single Side")
                    tvTests.Nodes(iNode).Nodes.Add("Dual Side")
                    tvTests.Nodes(iNode).Nodes.Add("Encode Only")
                    iNode += 1
                ElseIf (contactReader.ToLower().Contains("scm") OrElse contactReader.ToLower().Contains("identive")) Then
                    tvTests.Nodes.Add("Contact")
                    tvTests.Nodes(iNode).Nodes.Add("Single Side")
                    tvTests.Nodes(iNode).Nodes.Add("Dual Side")
                    tvTests.Nodes(iNode).Nodes.Add("Encode Only")
                    iNode += 1
    ...
    Private Sub tvTests_AfterSelect(ByVal sender As Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles tvTests.AfterSelect
        Try
            ...
            Select Case (fp)
                ...
                Case "Contact", "Contact\Encode Only", "Contact Station", "Contact Station\Encode Only"
                    If (_test <> "Contact") Then
                        cboCardTypesInit(TEST_TYPE.CONTACT)
                        _test = "Contact"
                    End If
                    _testChild = "Encode Only"
                    LblTestSet("Contact", "Encode Only")

                Case "Contact\Single Side", "Contact Station\Single Side"
                    If (_test <> "Contact") Then
                        cboCardTypesInit(TEST_TYPE.CONTACT)
                        _test = "Contact"
                    End If
                    _testChild = "Single Side"
                    LblTestSet("Contact", "Single Side")

                Case "Contact\Dual Side", "Contact Station\\Dual Side"
                    If (_test <> "Contact") Then
                        cboCardTypesInit(TEST_TYPE.CONTACT)
                        _test = "Contact"
                    End If
                    _testChild = "Dual Side"
                    LblTestSet("Contact", "Dual Side")
            End Select
        Catch ex As Exception
        Finally
            btnRun.Enabled = True
            ...
            For i As Integer = 0 To _cardTypes.Count - 1
                ...
                Else
                    Select Case (testType)
                        Case TEST_TYPE.CONTACT, TEST_TYPE.CONTACTwithBARCODE
                            If (cardType.ToLower().Contains("pvc,sle")) Then
                                cboCardTypes.Items.Add(cardType)
                                If (first) Then
...
#Region "Contact Tests"
    ...
    Private Sub PerformContactTest(ByVal cardType As String, ByVal continueCount As Integer)

        Dim scm As ZMotifOpsContactSCM = Nothing
        Dim acr38 As ZMotifOpsContactACR38 = Nothing
        Try
            Dim cardCount As Integer = 1
            Dim errCount As Integer = 0
            Dim actionID As Integer = 0
            ...
            While (True)
                ...
                If (IsPrinterReady(printerStatus, errMsg, alarm) OrElse _testChild = "Encode Only") Then

                    alarm = CheckAlarm()

                    If (_alarm = 0) Then

                        ConfigureContactTest(_testChild, cardType, actionID, _
                                             errMsg, scm, acr38)

                        TestInProgress = True

                        cardCount = DetermineNextStepContactTest(cardCount, cardType, errMsg, _
                                                                 actionID, alarm, _testChild, _
                                                                 errCount)
                    Else
                        alarm = CheckAlarmValue(alarm, cardCount)
            ...
            If Not (acr38 Is Nothing) Then
                acr38.ReleaseContext()
            End If

            cardCount -= 1

            LogAppend("End " + _test + " " + _testChild + ": Cards = " _
                      + cardCount.ToString() + " Errors = " + errCount.ToString())

        Catch ex As Exception
            MessageBox.Show(ex.Message, "PerformContactTests threw exception")
        Finally
            lblStatusUpdate("")
            scm = Nothing
            acr38 = Nothing
    ...
    Private Sub ConfigureContactTest(ByVal testType As String, ByVal cardType As String, _
                                     ByRef actionID As Integer, ByRef errMsg As String, _
                                     ByRef scm As ZMotifOpsContactSCM, ByRef acr38 As ZMotifOpsContactACR38)
        Try
            DetermineContactTestType(testType, cardType, actionID, errMsg)

            If (String.IsNullOrEmpty(errMsg)) Then

                SetupContactTest(errMsg, scm, acr38)

            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "ConfigureContactTest threw exception")
        End Try
    End Sub

    Private Sub DetermineContactTestType(ByVal testType As String, ByVal cardType As String, _
                                         ByRef actionID As Integer, ByRef errMsg As String)
        Try
            Select Case (testType)

                Case "Single Side"
                    ContactJob(actionID, cardType, _bmpFront, Nothing, errMsg)

                Case "Dual Side"
                    ContactJob(actionID, cardType, _bmpFront, _bmpBack, errMsg)

                Case Else 'Encode Only
                    ContactJob(actionID, cardType, Nothing, Nothing, errMsg)

            End Select
        Catch ex As Exception
            MessageBox.Show(ex.Message, "DetermineContactTestType threw exception")
        End Try
    End Sub

    Private Sub SetupContactTest(ByRef errMsg As String, _
                                 ByRef scm As ZMotifOpsContactSCM, _
                                 ByRef acr38 As ZMotifOpsContactACR38)
        Dim smartCardType As String = String.Empty
        Dim atr As String = String.Empty

        Try
            Dim key As Byte() = {&HFF, &HFF, &HFF, &HFF, &HFF, &HFF}

            If (_contactReader.ToLower().Contains("scm")) Then

                If (scm Is Nothing) Then
                    scm = New ZMotifOpsContactSCM()
                End If

                'perform contact test
                scm.ContactTest(smartCardType, _contactReader, key, True, errMsg)
            Else
                If (acr38 Is Nothing) Then
                    acr38 = New ZMotifOpsContactACR38()
                End If

                smartCardType = cboCardTypes.Text

                If (smartCardType.Contains("SLE4442")) Then
                    smartCardType = "SLE4442"
                ElseIf (smartCardType.Contains("SLE5542")) Then
                    smartCardType = "SLE5542"
                ElseIf (smartCardType.Contains("SLE4428")) Then
                    smartCardType = "SLE4428"
                ElseIf (smartCardType.Contains("SLE5528")) Then
                    smartCardType = "SLE5528"
                Else
                    smartCardType = "Unknown"

                    'perform contact test
                    acr38.ContactTest(smartCardType, _contactReader, key, _
                                      atr, True, errMsg)
                End If
            End If
        ...
        Catch ex As Exception
            MessageBox.Show(ex.Message, "SetupContactTest threw exception")
        End Try
    End Sub

    Private Function DetermineNextStepContactTest(ByVal cardCount As Integer, ByVal smartCardType As String, _
        ...
        Try
            ...
            If (String.IsNullOrEmpty(errMsg)) Then
                ...
                ResumeJob()
                JobWait(actionID, TEST_TYPE.CONTACT, 60, errMsg, alarm, (type = "Encode Only"))

                If ((String.IsNullOrEmpty(errMsg) OrElse errMsg = "done_ok" OrElse errMsg = "card exiting to eject bin" _
                    OrElse errMsg = "card exiting to reject bin" OrElse errMsg = "card exiting to feeder") _
                    AndAlso ((alarm = 0) OrElse (alarm = 4016))) Then

                    logMsg = smartCardType + " Contact Passed: Card " + cardCount.ToString()
                    LogAppend(logMsg)
                ElseIf (alarm = CARD_NOT_DETECTED) Then
                    ...
                    errCount += 1
                Else
                    logMsg = smartCardType + " Job Wait Error: " + errMsg + ": Card " _
                            + cardCount.ToString()
                    LogAppend(logMsg)
                    errCount += 1
                End If
            ...
            Else
                logMsg = "Contact Test Error: " + errMsg + ": Card " + cardCount.ToString()
                LogAppend(logMsg)
                errCount += 1
                TerminateJob()
            End If
        ...
        Catch ex As Exception
            errCount += 1
            LogAppend("DetermineNextStepContactTest threw exception: " + ex.Message)
        Finally
            cardCount += 1
        End Try
...
#Region "ZMotifPrinter SDK"
    ...
    '           isContact     - flag indicating contact reader installed
    '           isMag         - flag indicating magnetic encoding print head
    '           errMsg        - returns error message if error occurs
    '  Outputs: None 
    '***************************************************
    Private Sub GetPrinterConfiguration(ByVal deviceName As String, ByRef isContact As Boolean, _
        ...
        Try
            ...
            If (OpenConnectionToPrinter()) Then
                ...
                If (String.IsNullOrEmpty(errMsg)) Then
                    ...
                    If (Not String.IsNullOrEmpty(contactEncoder)) Then
                        isContact = True
                    End If
                End If
    ...
    '  Name:    ContactJob
    '  Purpose: Demonstrates how to perform single and 
    '           dual sided printing with contact
    '           smart card
    '
    '  Inputs:  actionID - returns print job ID
    '           cardType - type of card being used
    ...
    '           False - Job failed
    '***************************************************
    Public Function ContactJob(ByRef actionID As Integer, ByVal cardType As String, _
        ...
        Try
            ...
            _job.JobControl.SmartCardConfiguration(SideEnum.Front, SmartCardTypeEnum.Contact, True)
        ...
        Catch e As Exception
            errMsg = "Contact Smart Card Job Error: " + e.Message
        Finally
            _job.ClearGraphicsLayers()
            GC.Collect()
        End Try
    ...
    Private Sub JobWait(ByVal actionID As Integer, ByVal testType As TEST_TYPE, ByVal loops As Integer, _
        ...
        While (loops > 0)
            ...
            Select Case (testType)
                ...
                Case TEST_TYPE.CONTACT
                Case TEST_TYPE.CONTACTwithBARCODE
                    If (contactStatus.ToLower().Contains("error")) Then
