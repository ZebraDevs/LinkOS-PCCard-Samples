...
namespace ZXP7_ZXP8_CSharp_PrintUV_Panel_SA
{
    class ZXPSampleCode
    {
        #region Declarations
        ...
        public bool Perform_Inhibit_Panel_Print(string cardType)
        {
            bool bRet = true;
            ...
            try
            {
                ...
                g.DrawTextString(50.0f, 580.0f, "Color + Inhibit Panel Print: Background fill, then Image", "Arial", 10.0f,
                ...
                //re-init graphics for Inhibit panel
                g.InitGraphics(0, 0, ZMotifGraphics.ImageOrientationEnum.Landscape,
                               ZMotifGraphics.RibbonTypeEnum.Inhibit, 0xCCCCCC);

                //load image to be printed as Inhibit panel:
                img = g.ImageFileToByteArray("Zebra.bmp");
                g.DrawImage(ref img, 50, 50, 924, 548, 0);

                dataLen = 0;
                uvBmp = g.CreateBitmap(out dataLen);

                // Print image on both sides + Inhibit Panel
                // -------------------------
                ...
                // Inhibit Panel
                job.BuildGraphicsLayers(SideEnum.Back, PrintTypeEnum.Inhibit, 0, 0, 0, -1, GraphicTypeEnum.BMP, uvBmp);
